...
interface IFaultDisputeGame is IDisputeGame {
    ...
    function getChallengerDuration(
        uint256 _claimIndex
    ) external view returns (Duration duration_);
    ...
    function getRequiredBond(
        Position _position
    ) external view returns (uint256 requiredBond_);
    ...
    function resolutionCheckpoints(
        uint256
    )
        external
        view
    ...
    function startingOutputRoot()
        external
        view
        returns (Hash root, uint256 l2BlockNumber); // nosemgrep
